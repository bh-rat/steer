...
The loop is an enforced flow; each stage verifies against its artifact
...
2. **draft**: read the input, identify every pattern above, and write
   ...
   original covers.
3. **audit**: ask "What makes the below so obviously AI generated?"
   about your own draft and write the remaining tells, briefly, to
   `out/humanize/tells.md`.
4. **final**: revise into `out/humanize/final.md`, addressing every
   tell. The step completes only when the file exists AND contains no
   em or en dashes.

Deliver the draft, the tells, the final rewrite, and (optionally) a
short summary of changes. To see the loop end to end on real text,
read `references/full-example.md`.
